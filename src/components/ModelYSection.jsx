...
function ModelYSection({ products }) {
    return (
        <div>
            {products.filter(car => car.title === "Model Y").map(filteredCar => (
                <section className="hero-section" key={filteredCar.id}
                    ...
                    <div className="hero-content">
                        ...
                        <div className="btn">
                            <Link to="OrderModelY"><button className='left-btn v-btn'><LeftButton leftText="Order Now" /></button></Link>
                            <button className='right-btn v-btn'><RightButton rightText="Demo Drive" /></button>
                        </div>
                        <SectionFooter totalPrice="50,490" />
                    </div>
                </section>
            ))}
